...
**単一ファイルで全機能を実装したフルバージョン（1,700行超）**
...
- ✅ 取引先管理
- ✅ 初期セットアップ（シート自動作成）
- ✅ **PDF自動生成**（シート作成方式・設定不要）
- ✅ **PDFシート削除機能**（チェックボックスで一括削除）
...
2. 書類種別と書類番号を選択
3. 「PDF生成」ボタンをクリック
4. **新しいシート（`PDF_書類番号`）が作成されます**
5. PDFダウンロードリンクが表示されるのでクリック
6. PDFがダウンロードされます！

#### PDFシートの管理

- 生成されたシートは `PDF_Q-20250101-001` のような名前で保存されます
- シートタブをクリックすると、書類をプレビューできます
- 不要なシートを削除する場合：
  1. メニュー「📄 請求書管理」→「🗑️ PDFシートを削除」をクリック
  2. 削除したいシートをチェック
  3. 「削除」ボタンをクリック

#### 💡 ポイント

- ✅ **Google Docsテンプレート不要**（旧方式は廃止しました）
- ✅ **すぐに使える**（設定ゼロ）
- ✅ **プレビュー可能**（シートで確認できる）
- ✅ **簡単削除**（チェックボックスで一括削除）
