<?php
/**
 * BNI Slide System - 更新メンバー管理画面 (Admin Only)
 * 管理者専用 - 更新メンバー選択管理
 */
...
<head>
  ...
  <title>更新メンバー管理 | BNI Slide System</title>
...
<body>
    ...
    <div class="container">
      ...
      <nav class="site-nav">
        <ul>
          ...
          <li><a href="renewal_members_admin.php" class="active">更新メンバー</a></li>
  ...
  <main class="main-content">
    <div class="container">
      <div class="admin-container">
        <div class="card">
          <h1>更新メンバー管理</h1>
          <p class="mb-3">更新対象のメンバーを選択します。</p>
  ...
  <script>
    ...
    async function loadData() {
      ...
      try {
        const response = await fetch('../api_load_renewal_members.php?week_date=' + weekDate);
        const data = await response.json();
        ...
        if (data.success && data.data) {
          ...
          // 既存データをチェック
          const memberIds = JSON.parse(data.data.member_ids || '[]');
    ...
    document.getElementById('renewalForm').addEventListener('submit', async (e) => {
      ...
      formData.append('member_ids', JSON.stringify(checkedMembers));
      formData.append('notes', notes);

      try {
        const response = await fetch('../api_save_renewal_members.php', {
          method: 'POST',
          body: formData
        });
