...
**単一ファイルで全機能を実装したフルバージョン（1,500行超）**
...
- ✅ 取引先管理
- ✅ 初期セットアップ（シート自動作成）
- ✅ **PDF自動生成**（Google Docsテンプレート）
- ✅ **テストデータ自動挿入**（すぐに動作確認可能）
- ❌ Gmail連携なし（必要に応じて追加可能）
...
1. メニュー「📄 請求書管理」→「🧪 テストデータを挿入」をクリック
2. 自社情報・取引先3件・見積書2件・請求書1件が自動挿入されます
3. すぐに動作確認ができます

### 6. PDF生成の設定（任意・推奨）

PDFを生成するには、Google Docsテンプレートを作成する必要があります。

#### ステップ1: テンプレートを作成

1. Google Docsで新規ドキュメントを作成
2. 以下のようなレイアウトで請求書テンプレートを作成：
...
3. 見積書・納品書・請求書・領収書の4種類作成（内容は同じでOK）

#### ステップ2: テンプレートIDを取得

1. Google DocsのURLから ID部分をコピー
   ```
   https://docs.google.com/document/d/【ここがテンプレートID】/edit
   ```

#### ステップ3: コードに設定

1. Apps Scriptエディタで FULL_VERSION.gs を開く
2. 39-44行目の `TEMPLATE_IDS` を編集：

```javascript
const TEMPLATE_IDS = {
  QUOTE: '1AbC2dEfGhIjKlMnOpQr',        // 見積書テンプレートID
  DELIVERY: '2BcD3eFgHiJkLmNoPqRs',     // 納品書テンプレートID
  INVOICE: '3CdE4fGhIjKlMnOpQrSt',      // 請求書テンプレートID
  RECEIPT: '4DeF5gHiJkLmNoPqRsTu'       // 領収書テンプレートID
};
```
...
1. メニュー「📄 請求書管理」→「📥 PDFを生成」をクリック
2. 書類種別と書類番号を選択
3. 「PDF生成」ボタンをクリック
4. Google Driveに自動保存されます（「見積書PDF」「請求書PDF」等のフォルダ）
...
はい、FULL_VERSION.gsにPDF生成機能が含まれています。Google Docsテンプレートを作成すれば、ワンクリックでPDF化できます（設定方法はREADME参照）。Gmail連携は含まれていませんが、必要に応じて追加可能です。
